...
from ..connection.device import HonDevice
from ..connection.handler.auth import HonAuthConnectionHandler
...
class HonAuth:
    ...
    async def _manual_redirect(self, url: str) -> str:
        # `url` can already be the "hon://...oauth/done#access_token=..."
        # deep link here - e.g. _introduce() can match it directly out of
        # the page when the session is already authenticated (a shared
        # aiohttp session that just logged in moments earlier, as happens
        # between config-flow validation and the real setup_entry call).
        # It must never be fetched over HTTP.
        if "oauth/done#access_token=" in url:
            return url
    ...
    async def _handle_redirects(self, login_url: str) -> str:
        ...
        self._raise_if_already_authenticated(redirect1)
        redirect2 = await self._manual_redirect(redirect1)
        self._raise_if_already_authenticated(redirect2)
        return f"{redirect2}&System=IoT_Mobile_App&RegistrationSubChannel=hOn"

    def _raise_if_already_authenticated(self, url: str) -> None:
        # When the session is already authenticated (e.g. cookies from a
        # prior attempt on a shared aiohttp session), the OAuth server can
        # skip the login form entirely and redirect straight to the
        # "hon://...oauth/done#access_token=..." mobile deep link after
        # fewer hops than usual. That URL is not fetchable over HTTP - it
        # must be parsed for the tokens instead of followed.
        if "oauth/done#access_token=" in url:
