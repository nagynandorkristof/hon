...
from ... import const
from ...connection.handler.base import ConnectionHandler
from ...exceptions import HonConnectionError
from ...typedefs import Callback
        ...
        try:
            async with method(url, *args, **kwargs) as response:
                self._called_urls.append(
                    (response.status, str(response.request_info.url))
                )
                yield response
        except (aiohttp.ClientError, TimeoutError) as error:
            raise HonConnectionError(str(error)) from error
