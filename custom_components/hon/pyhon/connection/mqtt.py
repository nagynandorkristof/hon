...
class MQTTClient:
    ...
    def _on_publish_received(self, data: mqtt5.PublishReceivedData) -> None:
        try:
            self._handle_publish_received(data)
        except Exception:
            _LOGGER.exception("Failed to handle mqtt publish")

    def _handle_publish_received(self, data: mqtt5.PublishReceivedData) -> None:
        ...
        while True:
            ...
            if not self._connection:
                try:
                    ...
                    _LOGGER.exception("Failed to restart mqtt connection")
